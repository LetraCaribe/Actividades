...
// -- LegoLesson (stepper) -----------------------------------
// Leccion multi-paso ENCIMA del motor (blueprint-legolesson.md). Cada elemento
...
function LegoVocabularyEntryForm(opts) {
  ...
  if (!document.getElementById('lego-vocabulary-entry-form-styles')) {
    ...
    st.textContent = '.lego-vocab-form{display:flex;flex-direction:column;gap:14px;color:var(--ink,#1C1A17)}'
      + '.lego-vocab-top{display:grid;grid-template-columns:minmax(0,2fr) minmax(150px,1fr);gap:12px}'
      ...
      + '.lego-vocab-senses-head{display:flex;align-items:center;justify-content:space-between;gap:10px}'
      ...
      + '.lego-vocab-count{font-size:11px;color:var(--muted,#8C8479)}'
      ...
      + '.lego-vocab-sense-head{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:9px}'
      + '.lego-vocab-sense-name{font-size:11px;font-weight:800;text-transform:uppercase;color:var(--muted,#8C8479)}'
      + '.lego-vocab-sense-details{display:grid;grid-template-columns:1fr;gap:9px;margin-top:10px;padding-top:10px;border-top:1px solid var(--border,#DDD7CE)}'
      + '.lego-vocab-link{align-self:flex-start;border:0;background:transparent;padding:2px 0;color:var(--teal,#1E6B72);font:inherit;font-size:12px;font-weight:700;cursor:pointer}'
      ...
      + '@media(max-width:560px){.lego-vocab-top{grid-template-columns:1fr}}';
    document.head.appendChild(st);
  }

  var root = document.createElement('div'); root.className = 'lego-vocab-form';
  var top = document.createElement('div'); top.className = 'lego-vocab-top';
  ...
  var type = LegoSelect({
    className: 'lego-vocab-control',
    value: value.entry_type || 'word',
    options: [
  ...
  function updateRows(){
    rows.forEach(function(row, i){
      row.name.textContent = 'Sentido ' + (i + 1);
      row.remove.style.visibility = (!opts.readOnly && !row.locked && rows.length > 1) ? 'visible' : 'hidden';
    });
    count.textContent = rows.length + (rows.length === 1 ? ' sentido' : ' sentidos');
  }
  ...
  function addSense(data){
    ...
    var head = document.createElement('div'); head.className = 'lego-vocab-sense-head';
    var name = document.createElement('div'); name.className = 'lego-vocab-sense-name';
    var remove = LegoButton({ variant: 'ghost', size: 'sm', icon: 'ti-trash', title: 'Eliminar sentido' });
    head.replaceChildren(name, remove);
    ...
    var detailsButton = document.createElement('button'); detailsButton.type = 'button'; detailsButton.className = 'lego-vocab-link';
    var details = document.createElement('div'); details.className = 'lego-vocab-sense-details';
    var context = document.createElement('input'); context.className = 'lego-vocab-control'; context.value = data.context || ''; context.placeholder = 'ej. permanecer hasta que algo ocurra'; context.disabled = locked;
    ...
    var alternatives = document.createElement('input'); alternatives.className = 'lego-vocab-control'; alternatives.value = (data.accepted_answers || []).join('; '); alternatives.placeholder = 'ej. poner música; reproducir'; alternatives.disabled = locked;
    details.appendChild(LegoField({ label: 'Contexto o pista', control: context, optional: true, compact: true }));
    details.appendChild(LegoField({ label: 'Ejemplo', control: example, optional: true, compact: true }));
    details.appendChild(LegoField({ label: 'Otras respuestas aceptadas', control: alternatives, optional: 'separadas por ;', compact: true }));
    var expanded = !!(data.context || data.example || (data.accepted_answers || []).length);
    function drawDetails(){
      details.style.display = expanded ? 'grid' : 'none';
      detailsButton.textContent = expanded ? 'Ocultar contexto, ejemplo y alternativas' : '＋ Añadir contexto, ejemplo o alternativas';
    }
    detailsButton.onclick = function(){ if (locked) return; expanded = !expanded; drawDetails(); };
  ...
  root.getValue = function(){
    return {
      ...
      entry_type: type.value || 'word',
      senses: rows.map(function(row){
        return {
          bank_sense_id: row.bank_sense_id,
          locked: row.locked,
          meaning: row.meaning.value.trim(),
          context: row.context.value.trim(),
          example: row.example.value.trim(),
          accepted_answers: row.alternatives.value.split(';').map(function(a){ return a.trim(); }).filter(function(a){ return a; })
        };
      })
    };
  };
